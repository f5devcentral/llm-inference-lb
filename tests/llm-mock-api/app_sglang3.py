...
def update_metrics():
    while True:
        if DYNAMIC_UPDATE_ENABLED:
            metrics["num_requests_waiting"] = round(random.uniform(800.0, 2000.0), 1)
            metrics["gpu_cache_usage_perc"] = round(random.uniform(0.8, 1.0), 2)
            metrics["num_running_reqs"] = round(random.uniform(60.0, 900.0), 1)
