...
**关键点**：
- **变种命名规则**：必须以`vllm`或`sglang`开头（下划线或中划线均可，如`vllm_ascend`、`vllm-mlu`）
- **Pool的engine_type配置**：始终使用基础类型（`vllm`或`sglang`），而非变种名称
- **可选配置**：仅配置与内置标准不同的key；未配置的key自动回退使用内置默认值
- **优先级**：用户配置的变种key优先尝试，找不到时回退到内置key
- **自动检测**：调度器根据可用的metrics自动检测每个member使用的变种
- **热加载**：`engines_metrics_keys`的修改会热加载，在下一个metrics采集周期生效

**API响应**：`/pools/{pool_name}/{partition}/status`接口在每个member信息中包含`detected_variant`字段，显示检测到的变种名称（如`vllm_ascend`、`vllm`、`sglang_xxx`）
